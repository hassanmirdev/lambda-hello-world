...
resource "aws_apigatewayv2_integration" "hello_world_LF" {
  api_id             = aws_apigatewayv2_api.lambda.id
  integration_uri    = var.lambda_invoke_arn  # Update this with the new Lambda ARN if necessary
  integration_type   = "AWS_PROXY"
  integration_method = "POST"
}

# Route to trigger Lambda
resource "aws_apigatewayv2_route" "hello_world_LF" {
  api_id   = aws_apigatewayv2_api.lambda.id
  route_key = var.route_key
  target   = "integrations/${aws_apigatewayv2_integration.hello_world_LF.id}"
}
...
resource "aws_lambda_permission" "api_gw" {
  ...
  function_name = "hello-world-LF"  # Updated Lambda function name
